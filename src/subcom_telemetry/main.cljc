(ns subcom-telemetry.main
  "Kotodama WASM entrypoint for the Subcom_telemetry clean-room actor (L4) — Clojure port.
  ...
  facts (`subcom_telemetry.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "subcom_telemetry")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"subcom_telemetry.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "subcom_telemetry-compat" :tier tier :entities entities} 200])
...
(defn start! [] :subcom_telemetry-compat/ready)
