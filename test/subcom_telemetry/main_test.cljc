(ns subcom-telemetry.main-test
  "Contract + behavioral test for the subcom-telemetry L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "subcom_telemetry-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'subcom-telemetry.main-test)]
